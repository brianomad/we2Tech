...
const TXS = [
  { type: 'in', who: '0x3f8a\u2026d21c', amt: '+0.42 ETH', time: '2 min ago', color: S.green, icon: 'arrowDown' },
  { type: 'out', who: '0x91c2\u2026bb0e', amt: '\u22120.08 ETH', time: '1 hr ago', color: S.red, icon: 'arrowUp' },
  { type: 'swap', who: 'Uniswap V3', amt: '250 wTKN', time: 'Yesterday', color: S.purple, icon: 'refresh' },
  { type: 'mint', who: 'NovaChain', amt: '+0.05 ETH', time: '2 days ago', color: S.cyan, icon: 'sparkle' },
  { type: 'in', who: '0x7a11\u2026e09f', amt: '+0.15 ETH', time: '3 days ago', color: S.green, icon: 'arrowDown' },
  { type: 'in', who: '0x5d90\u2026c332', amt: '+0.06 ETH', time: '4 days ago', color: S.green, icon: 'arrowDown' },
];
...
export default function BlockchainDemo({ t, item }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setSent(true);
      setTxList((ls) => [
        { type: 'out', who: to, amt: `\u2212${amt} ETH`, time: 'Just now', color: S.red, icon: 'arrowUp' },
        ...ls,
      ]);
      ...
      <AppBar
        ...
        right={
          <Btn
            ...
            onClick={connect}>
            {loading ? d.connect + '\u2026' : connected ? <><StatusDot color="#4ADE80" /> {d.connected}</> : `+ ${d.connect}`}
          </Btn>
        }
      />
      ...
      <Box sx={{ background: 'radial-gradient(circle at 70% 0%, #1E1B4B 0%, #0B1220 60%)', p: [3, null, 4], minHeight: 470, display: 'flex', flexDirection: 'column', position: 'relative' }}>
        {!connected ? (
          <Card sx={{ p: 6, textAlign: 'center', background: 'rgba(255,255,255,0.04)', border: '1px dashed rgba(255,255,255,0.18)', maxWidth: 420, mx: 'auto', my: 'auto' }}>
            <Box sx={{ width: 72, height: 72, mx: 'auto', borderRadius: '50%', background: 'linear-gradient(135deg,#8B5CF6,#EC4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 3, boxShadow: '0 18px 40px rgba(139,92,246,0.45)' }}>
            ...
            <Btn tone="primary" sx={{ backgroundColor: '#8B5CF6', backgroundImage: 'linear-gradient(180deg, rgba(255,255,255,0.2), transparent 55%)', boxShadow: '0 10px 24px rgba(139,92,246,0.45)' }} onClick={connect}>
              {loading ? <><Icon name="clock" size={15} /> {d.connect}&hellip;</> : `+ ${d.connect}`}
            </Btn>
          </Card>
        ) : (
            <Box sx={{ display: 'grid', gridTemplateColumns: ['1fr', null, '1fr 1.7fr'], gap: 4, '@container (max-width: 700px)': { gridTemplateColumns: '1fr' } }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              <Card sx={{ p: 4, background: 'linear-gradient(135deg,#7C3AED,#4C1D95)', border: 'none', boxShadow: '0 18px 40px rgba(124,58,237,0.4)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
                  ...
                  <Box sx={{ color: 'rgba(255,255,255,0.85)', display: 'flex' }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, '@container (max-width: 460px)': { gridTemplateColumns: '1fr' } }}>
                ...
                <Card
                  ...
                  <Box sx={{ width: 44, height: 44, borderRadius: 13, background: 'linear-gradient(135deg,#22D3EE,#3B82F6)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 18px rgba(34,211,238,0.35)' }}>
                ...
                <Card sx={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' }}>
                  {txList.map((tx, i) => (
                    <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 3, px: 3, py: 2.5, borderBottom: '1px solid rgba(255,255,255,0.08)', ':last-child': { border: 'none' }, '&:hover': { backgroundColor: 'rgba(255,255,255,0.04)' } }}>
                      <Box sx={{ width: 36, height: 36, borderRadius: 10, backgroundColor: `${tx.color}22`, color: tx.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            {view === 1 && (
              <Box>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3, '@container (max-width: 460px)': { gridTemplateColumns: '1fr 1fr' } }}>
                  {NFTS.map((nft) => (
                    <Card key={nft.id} sx={{ overflow: 'hidden', cursor: 'pointer', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 16px 34px rgba(0,0,0,0.4)' }, transition: 'all 0.2s' }}>
                      <Box sx={{ height: 88, background: nft.grad, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        ...
                        <Box sx={{ color: '#fff', display: 'flex', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.4))' }}>
            ...
            {view === 2 && (
              <Card sx={{ p: 5, textAlign: 'center', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
                <Box sx={{ width: 64, height: 64, mx: 'auto', borderRadius: '50%', background: 'rgba(139,92,246,0.2)', color: '#A78BFA', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 3 }}>
                  <Icon name="barChart" size={28} />
                </Box>
                <Text sx={{ display: 'block', fontWeight: 700, color: '#fff', fontFamily: font }}>{d.gallery} &amp; {d.stake}</Text>
                <Text sx={{ display: 'block', fontSize: 0, color: 'rgba(255,255,255,0.5)', fontFamily: font, mt: 1 }}>{d.analyticsView}</Text>
              </Card>
            )}
          </Box>
        )}
          ...
          <Box sx={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20, p: 3 }}>
            <Card sx={{ p: 4, maxWidth: 420, width: '100%' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                ...
                {!confirming && (
                  <Box onClick={() => setSendOpen(false)} sx={{ cursor: 'pointer', color: S.muted, display: 'flex' }}>
                    <Icon name="x" size={18} />
                  </Box>
                )}
              </Box>
              {!confirming ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  <Btn tone="primary" sx={{ backgroundColor: '#8B5CF6', backgroundImage: 'linear-gradient(180deg, rgba(255,255,255,0.2), transparent 55%)', width: '100%', mt: 1 }} onClick={() => setConfirming(true)}>
                    {d.sendCta}
                  </Btn>
                </Box>
              ) : (
        ...
        {sent && (
          <Box sx={{ position: 'absolute', right: 3, bottom: 14, zIndex: 21, display: ['none', null, 'block'] }}>
            <Toast tone="dark">
              <Box sx={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(74,222,128,0.2)', color: '#4ADE80', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
